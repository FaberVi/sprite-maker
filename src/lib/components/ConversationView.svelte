<script lang="ts">
  ...
  function readableContent(message: Message, hasArtifact: boolean): string {
    ...
    if (message.role !== "assistant" || !["claude", "gemini", "grok", "cursor"].includes(provider?.id ?? "")) return value;
